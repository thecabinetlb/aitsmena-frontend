...
<template>
    <section class="w-11/12 h-full mx-auto 2xl:w-8/12 lg:w-10/12">
        ...
        <div class="grid relative w-full justify-center items-center grid-cols-2 md:grid-cols-4 2xl:grid-cols-7 gap-6 md:mb-[194px] mt-10 max-md:pb-10"> 
            ...
            <!-- Desktop -->
            <p v-for="(item, key) in data" :key="key" v-show="openDetails === key" class="md:block absolute hidden 2xl:left-1/2 2xl:transform 2xl:-translate-x-1/2 2xl:w-9/12 2xl:px-6 top-[100%] my-10 md:h-[194px] font-[200] max-sm:text-[14px] text-justify text-accent2" style="text-align-last: center;">{{ item.description }}</p>
        </div>
        <!-- Mobile -->
