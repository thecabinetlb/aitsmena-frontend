...
<template>
    <section class="w-11/12 mx-auto mt-40 2xl:w-8/12 sm:w-10/12">
        <h1 class="text-accent1 font-[200] text-lg uppercase"> &#x2022; Upcomming Events</h1>
        ...
        <ul role="list" class="flex flex-wrap justify-center lg;justify-between gap-10 lg:gap-3 list-style-none">
            <li v-for="(item, key) in data" :key="key" class="sm:w-[49%] cursor-pointer relative group max-sm:min-h-[440px]">
            <div class="w-full h-full max-sm:min-h-[440px] absolute -inset-0 bg-gradient-to-br from-accent1/10 to-accent1/30 group-hover:from-bg2/50 group-hover:to-bg2 rounded-[16px] opacity-35 group-hover:opacity-50 transform duration-400 group-hover:duration-600"></div>
                <div class="p-3 w-full h-full relative flex justify-between items-center lg:gap-3 gap-6 rounded-[16px] bg-noise bg-opacity-10 bg-fill bg-center border border-bg2">
                    ...
                    <div class="w-3/4">
                        ...
                        <!-- <RouterLink :id="'go-to-' + item.title + '-page'" :aria-label="'apply for' + item.title" :to="item.to" :target="item.target" class="ms-auto me-0 block w-fit cursor-pointer px-4 py-3 max-sm:text-[14px] font-[200] text-center rounded-[8px] shadow-lg text-accent1 bg-bg2 hover:brightness-125">Secure Your Spot</RouterLink> -->
                    </div>
                    <svg width="70" height="70" viewBox="0 0 70 70" class="w-1/4 aspect-square" fill="none" xmlns="http://www.w3.org/2000/svg">
