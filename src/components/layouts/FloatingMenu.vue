<script setup>
import icon1 from '../../assets/images/socials/twitter.svg'
import icon2 from '../../assets/images/socials/instagram.svg'
import icon3 from '../../assets/images/socials/discord.svg'
import { ref } from 'vue'
...
const sociallinks = [
{ id: 1, name:'Twiiter(X)', to: '/', target:"_blank", icon:icon1 },
{ id: 2, name:'Instagram', to: '/', target:"_blank", icon:icon2 },
{ id: 3, name:'Discord', to: '/', target:"_blank", icon:icon3 }
];
</script>
<style>
...
<template>
    ...
    class="fixed right-0 z-10 flex flex-col items-center justify-center w-12 h-10 transform -translate-x-1/2 border border-r-0 rounded-l-full -me-5 top-1/2 border-accent1/30 bg-accent1/30 text-accent1"
    @click="toggleModal">
    ...
    <Teleport to="#portals">
        ...
        <div v-if="isOpen" class="w-80 aspect-square py-2 px-6 fixed z-10 right-0 transform top-1/2 -translate-y-1/2 rounded-[16px]">
            <img src="../../assets/noise/noisy-gradients.svg" alt="AITS" width="418" height="512" class="absolute inset-0 z-[-1] rounded-[16px] w-full" responsive/>
            ...
            <ul role="list" class="space-y-4 list-style-none h-2/4">
                <li class="text-accent1 font-[200] flex gap-2">
                    ...
                    <path d="M10 5.49999C12.103 5.49999 13 6.39699 13 8.49999H15C15 5.27499 13.225 3.49999 10 3.49999V5.49999ZM13.422 10.943C13.2299 10.7683 12.9774 10.6752 12.7178 10.6832C12.4583 10.6912 12.212 10.7998 12.031 10.986L9.63804 13.447C9.06204 13.337 7.90404 12.976 6.71204 11.787C5.52004 10.594 5.15904 9.43299 5.05204 8.861L7.51104 6.46699C7.69745 6.28612 7.80617 6.03982 7.8142 5.7802C7.82223 5.52059 7.72892 5.26804 7.55404 5.07599L3.85904 1.01299C3.68408 0.820352 3.44092 0.703499 3.18119 0.687255C2.92146 0.67101 2.66564 0.756653 2.46804 0.925995L0.298038 2.78699C0.125149 2.96051 0.0219574 3.19145 0.00803782 3.43599C-0.00696218 3.68599 -0.292962 9.608 4.29904 14.202C8.30504 18.207 13.323 18.5 14.705 18.5C14.907 18.5 15.031 18.494 15.064 18.492C15.3086 18.4783 15.5394 18.3747 15.712 18.201L17.572 16.03C17.7415 15.8325 17.8273 15.5768 17.8113 15.317C17.7952 15.0573 17.6785 14.8141 17.486 14.639L13.422 10.943Z" fill="white"/>
                    </svg>
                    +1012 3456 789
                </li>
                <li class="text-accent1 font-[200] flex gap-2">
            ...
            <div class="flex flex-wrap items-center justify-between gap-2 place-content-end h-1/4">
                ...
                <a v-for="(item, key) in sociallinks" :key="key" :id="'go-to-' +item.name" :href="item.to" :target="item.target" class="w-10 aspect-square font-[200] bg-bg2 text-accent1 hover:text-bg2 hover:bg-accent1 rounded-full p-1 flex items-center justify-center">
                    <img :src="item.icon" :alt="'AITS' + item.name" responsive/>
                </a>                    
                </div>
