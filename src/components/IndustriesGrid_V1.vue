...
<template>
    <section class="w-11/12 h-full mx-auto 2xl:w-8/12 lg:w-10/12">
        ...
        <div class="relative grid w-full justify-center items-center grid-cols-1 gap-6 mb-[135px] mt-10 2xl:grid-cols-6 sm:grid-cols-3">
            ...
            <div v-for="(item, key) in data" :key="key" class="absolute left-0 top-[100%] mt-6 h-[135px]">
                <p v-show="openDetails === key" class="font-[200] max-sm:text-[14px] text-justify text-accent2">{{ item.description }}</p>
            </div>
        </div>
    </section>
</template>