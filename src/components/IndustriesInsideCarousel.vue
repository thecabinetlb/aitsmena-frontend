...
<template>
  <section>
    ...
    <Swiper
      ...
      :modules="[Autoplay]"
    >
      <SwiperSlide v-for="(item, key) in data" :key="key" 
        ...
        @click="stopAutoplay(key)"
        :class="{'scale-110': openDetails === key, 'scale-90': openDetails != key}">
    ...
    <div  class="w-11/12 pt-10 mx-auto 2xl:w-8/12 sm:w-10/12">
      ...
      <Transition name="slide-fade">
          <div v-if="openDetails === key" class="pb-10 mt-10 space-y-6 traking-wide text-accent2" v-html="item.body"></div>       
      </Transition>
        </div>
    </div>    
