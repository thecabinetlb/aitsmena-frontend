...
<template>
    <section class="w-11/12 mx-auto mt-40 2xl:w-8/12 lg:w-10/12">
      ...
      v-motion
        ...
        <img :src="item.image" :alt="item.title" width="391" height="290" center cover responsive loading="lazy" :class="{'order-2' : item.id % 2 === 0}"/>    
        <div class="w-full 2xl:w-3/5 lg:w-1/2">
          ...
          <RouterLink v-if="item.to" :id="'go-to-' + item.title + '-page'" :aria-label="'go to ' + item.title" :to="item.to" class="cursor-pointer px-4 py-3 w-1/3 block max-sm:text-[14px] font-[200] text-center rounded-[8px] text-accent1 bg-bg2 hover:brightness-125">Browse</RouterLink>
        </div>
      </div>
    </section>
</template>