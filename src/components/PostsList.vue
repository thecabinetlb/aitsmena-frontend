        ...
        <ul role="list" class="grid grid-cols-3 gap-6 mt-10 lg:gap-3 list-style-none">
            <li v-for="(item, key) in data" :key="key" class="flex-grow lg:col-span-1 ms:col-span-2 col-span-3 w-full relative group sm:min-h-[135px] rounded-[16px] border-2 border-bg2">
                ...
                <div class="flex flex-col gap-3 p-6 lg:gap-6">
                    ...
                    <RouterLink :id="'go-to-' + item.title + '-page'" :aria-label="'read more about' + item.title"  :to="'/resource-center/' + category + '/' + item.slug" class="cursor-pointer px-4 py-3 mt-auto mb-0 ms-auto me-0 w-fit block max-sm:text-[14px] font-[200] text-center rounded-[8px] text-accent1 bg-bg2 hover:brightness-125">Read More</RouterLink>
                </div>
            </li>
        </ul>
    </section>
