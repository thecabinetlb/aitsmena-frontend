...
import { getGraduateProgramJobs } from '../utils/laravel_api_functions'
import { onMounted, ref } from 'vue';

const graduateprogramjobs = ref([]);

const fetchData = async () => {
  graduateprogramjobs.value = await getGraduateProgramJobs();
  console.log(graduateprogramjobs.value);
};
onMounted(() => {
  fetchData();
});
...
<template>
    ...
    <section class="relative -mt-40">
      ...
      <JobsList sectionType="graduate program and internship opportunities" sectiontitle="Graduate Program and Internship Opportunities" 
      v-if="graduateprogramjobs.length > 0" :data="graduateprogramjobs"
